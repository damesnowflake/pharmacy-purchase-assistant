...
//  - 실사 기반 참고값(장중 실사 보정 포함)은 아직 이식하지 않았다. 최근 입고 기준 참고값만 계산한다.
...
Deno.serve(async (_req) => {
  ...
  const { data: queueRows, error: queueError } = await supabase
    ...
    .select("product_id, required_revision, need_training")
    .eq("status", "pending")
    .limit(BATCH_SIZE);
  if (queueError) {
    return new Response(`QUEUE_READ_ERROR: ${queueError.message}`, { status: 500 });
  }
  ...
  for (const row of queueRows) {
    ...
    try {
      ...
      if (model.status === "fitted") {
        ...
        // 최근 입고 기준 참고값 (실사 기준 경로는 아직 이식하지 않음 — 위 주석 참고)
        ...
        if (lastReceipt && lastReceipt.length > 0 && isCovered(today)) {
          const arrival = calculateArrivalDate(today, isCovered, (d) => holidaySet.has(d));
          if (arrival) {
            const B = (lastReceipt[0].occurred_at as string).slice(0, 10);
            ...
            const Q = (receiptsOnB ?? []).reduce((a, r) => a + (r.qty_base as number), 0);

            const actualRange = B <= today ? dateRange(B, today) : [];
            const actualSum = actualRange.reduce((acc, d) => acc + (dailyMap.get(d) ?? 0), 0);
            const forecastStart = B > addDays(today, 1) ? B : addDays(today, 1);
            const forecastSum = dateRange(forecastStart, arrival).reduce((acc, d) => acc + model.predict(d), 0);
            const arrivalDayEndExpected = Q - actualSum - forecastSum;

            if (arrivalDayEndExpected <= 0 && product.default_moq && product.default_order_step) {
              ...
              const basis = {
                reference_kind: "last_receipt",
                reference_value: arrivalDayEndExpected,
      ...
      await supabase
        .from("recompute_queue")
        .update({ status: "error", last_error: String(e), attempts: (row as { attempts?: number }).attempts ?? 1 })
        .eq("product_id", productId);
